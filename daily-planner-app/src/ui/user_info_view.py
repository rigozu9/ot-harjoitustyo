...
class UserInfoView:
    ...
    def __init__(self, master, user_id, user_service, daily_plan_service):
        ...
        self._pie_v=[self._sleep_fraction, self._outside_timefraction, self._productive_time_fraction, self._exercise_fraction, self._screen_time_fraction]
    ...
    def _create_piechart(self, pie_v, col_v):
        ...
        for val, col, label in zip(pie_v, col_v, activity_labels):
            ...
            self._canvas.create_arc(coord, start=st, extent=extent, fill=col, outline=col)
            ...
            text_y = center_y - text_offset * math.sin(angle_rad)  # Minus because the y-coordinates go down
            ...
            self._canvas.create_line(text_x, text_y, line_end_x, line_end_y, fill="white")
    ...
    def _display_info(self):
        ...
        self._gender_label = tk.Label(self._frame, text=f"Gender: {self._info[1]}")
        self._gender_label.pack()

        self._sleep_label = tk.Label(self._frame, text=f"Sleep goal: {self._info[2]/60} hours")
        self._sleep_label.pack()

        self._exericse_label = tk.Label(self._frame, text=f"Exercise goal: {self._info[3]/60} hours")
        self._exericse_label.pack()

        self._outside_label = tk.Label(self._frame, text=f"Outside goal: {self._info[4]/60} hours")
        self._outside_label.pack()

        self._productive_label = tk.Label(self._frame, text=f"Productive goal: {self._info[5]/60} hours")
        self._productive_label.pack()

        self._screentime_label = tk.Label(self._frame, text=f"Screentime goal: {self._info[6]/60} hours")
